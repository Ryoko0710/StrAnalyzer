...
struct OnlineTFBuilderNode : fair::mq::Device
{
    struct OptionKey {
    ...
    void InitTask() override
    {
        ...
        LOG(info) << "OnlineTFBuilderNode: InitTask Started. Channel: " << fInputChannelName << " Mode: " << fSamplingMode << " Prescale: " << fPrescale;

        gROOT->SetBatch(kTRUE);

        if (!gApplication) {
            static int argc = 1;
            static char* argv[] = {(char*)"OnlineTFBuilderNode", nullptr};
            new TApplication("OnlineTFBuilderNode", &argc, argv);
        }
...
{
    using opt = OnlineTFBuilderNode::OptionKey;
    options.add_options()
        (opt::InputChannelName.data(), bpo::value<std::string>()->default_value("in"), "Name of the input channel")
        (opt::SamplingMode.data(), bpo::value<std::string>()->default_value("latest"), "Sampling mode")
        (opt::Prescale.data(), bpo::value<int>()->default_value(1), "Prescale");
}

std::unique_ptr<fair::mq::Device> getDevice(fair::mq::ProgOptions& /*config*/)
{
    return std::make_unique<OnlineTFBuilderNode>();
}
